...
class SiteSpecificGlycan(object):
    ...
    def average(self):
        ...
        assert self.averaged == False, 'Cannot average glycan '+self.glycan+'; has already been averaged'
...
class PairedGlycan(object):
    ...
    def __init__(self, glycan, expected_site, experimental_site, expected_signal, experimental_signal):
        ...
        assert type(glycan) == OrderedDict, 'glycan is '+str(type(glycan))+', not OrderedDict'
        self.glycan = glycan
...
def kullback_leibler_divergence(p, q):
    ...
    assert len(p) == len(q)
    assert round(sum(p), 4) == 1, 'p is not a probability distribution; sum is '+round(sum(p), 4)
    assert round(sum(q), 4) == 1, 'q is not a probabiltiy distribution; sum is '+round(sum(q), 4)
...
def get_sequoninfodict_from_files_accid(csvfilelist, accessionid=None, seq='', score_cutoff=30, replicate_cutoff=2, sequon_length=4):
    ...
    'Replicate cutoff = '+str(replicate_cutoff)+' > '+str(len(csvfilelist))+' = number of csvs'
    ...
    site_to_glycans = prune_siteinfodict(site_to_glycans, replicate_cutoff=replicate_cutoff)
...
def get_distribution_axes(sequoninfodict):
    ...
    for sequon, sdict in sequoninfodict.items():
        ...
        for gcomp, signal in gcomp_to_signal.items():
            ...
            gdict = OrderedDict(sorted(gdict.items(), key=lambda x: tuple(x[0]==y for y in monosaccharide_order), reverse=True))
        ...
        axis = plot_bar_chart(norm_ordered_scores, ordered_glycan_strings, '', 'Probability', title, colors=colors)
...
def get_seq(accession_id, return_id=False):
    ...
    'query for accession id '+accession_id+' returned status code '+str(resp.status_code)
    
    # get sequence from returned text and return
    # seq will be all lines but first, since first line will be fasta carrot line
    seq = None
    if resp.text != '':
...
def update_siteinfodict(siteinfodict, csvcontent, replicate_id, accession_id, peptidesequence, score_cutoff=30):
    ...
    required_keys = set(['protein_name', 'glycopeptide', 'total_signal', 'peptide_start', 'peptide_end', 'ms2_score'])
    
    for i in csvcontent:
        # check required keys are present
        assert required_keys.issubset(i.keys()), 'row keys are '+str(i.keys())
        ...
        site = get_matching_nglycan_site(glycopeptide, start, end, peptidesequence)
        assert site in siteinfodict.keys(), 'site '+str(site)+' not in infodict keys '+str(siteinfodict.keys())
        ...
        if found:
            continue
        glycan = SiteSpecificGlycan(composition, site, accid, signal, [replicate_id], ms2_score)
...
def get_glycosite_from_glycopeptide_sequence(glycopeptide_sequence, start_position):
    ...
    assert len(splitseq) == 2, 'Multiple n-glycosylations on glycopeptide '+glycopeptide_sequence
    
    # take first part of AA sequence, remove other modifications, and rejoin again
    subseq = splitseq[0]
...
def get_matching_nglycan_site(gpep_sequence, gpep_start, gpep_end, peptidesequence):
    ...
    sites_from_protein = [ x for x in peptidesequence.n_glycan_sequon_sites if x >= gpep_start \
                           and x <= gpep_end ]
    site_from_peptide = get_glycosite_from_glycopeptide_sequence(gpep_sequence, gpep_start)
    ...
    ' != sites from pepseq = '+str(sites_from_protein)+' for glycopeptide '+gpep_sequence
...
def prune_siteinfodict(siteinfodict, replicate_cutoff=2):
    ...
    for site, glycanlist in siteinfodict.items():
        ...
        for g in glycanlist:
            ...
            str(round(g.total_signal, 3))+' != calculated average '+str(round(calc_average, 3))
...
def get_matched_sequon_infodict(expected_sequoninfodict, experimental_sequoninfodict):
    ...
    for sequon, expected_sequondict in expected_sequoninfodict.items():
        ...
        glycan_comps = set([ x.glycan for x in expected_glycans+experimental_glycans ])
        
        # for each glycan, turn into OrderedDict and build PairedGlycan object before adding to glist
        glist = []
        for gcomp in glycan_comps:
            # get signals for expected and experimental
            expected_signal = get_signal_by_composition(expected_glycans, gcomp)
            experimental_signal = get_signal_by_composition(experimental_glycans, gcomp)

            # transform gcomp into OrderedDict
            gdict = str_to_dict(gcomp.replace(';', ','))
            gdict = OrderedDict(sorted(gdict.items(), 
                                       key=lambda x: tuple(x[0]==y for y in monosaccharide_order), 
                                       reverse=True))
        ...
        ordered_glist = sorted(glist, key=lambda x: tuple(x.glycan[y] for y in glycan_order if y in x.glycan))
...
def get_comparison_calculation_dict(matched_dictionary, expected_distribution_name, experimental_distribution_name):
    ...
    for sequon, sequondict in matched_dictionary.items():
        ...
        expected_norm_signals = [ x/sum(expected_signals) for x in expected_signals ]
        experimental_norm_signals = [ x/sum(experimental_signals) for x in experimental_signals ]
        assert round(sum(expected_norm_signals), 4) == 1
        assert round(sum(experimental_norm_signals), 4) == 1

        # calculate cosine similarity and normalized Kullback-Leibler divergence
        cosine = cosine_similarity(expected_norm_signals, experimental_norm_signals)
        nkld = normalized_kullback_leibler_divergence(expected_norm_signals, experimental_norm_signals)

        # get comparison axis
        comparison_title = '{} Glycan Composition Comparision\nCosine similarity: {}\nnKLD: {}'.format(sequon, round(cosine, 3), round(nkld, 3))
...
def get_single_distribution_axis(probabilities, pairedglycans, charttitle, xtitle='Glycans', ytitle='Probability', width=0.35, color=True):
    ...
    assert round(sum(probabilities), 4) == 1, 'Not a probability distribution; sum is '+str(sum(probabilities))
...
def get_glycan_color(glycan, low='crimson', medium='purple', high='navy'):
    ...
    'which is not valid'
    assert 'HexNAc' in glycan.keys(), 'HexNAc not in glycan keys: '+str(glycan.keys())
...
def get_glycan_string(glycan, abbreviated=True):
    ...
    if abbreviated:
        ...
        s = ', '.join([ x+':'+y for x,y in zip(monosaccharide_order,counts) ])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-xc', '--expect-csv', dest='expect_csvs', action='append', required=True, help='CSV files for the expected distribution')
    parser.add_argument('-pc', '--exper-csv', dest='exper_csvs', action='append', required=True, help='CSV files for the experimental distribution')
    parser.add_argument('-xa', '--exect-accid', dest='expect_accid', required=True, help='accession ID of expected protein')
    parser.add_argument('-pa', '--exper_accid', dest='exper_accid', required=True, help='accession ID of experimental protein')
    parser.add_argument('-o', '--outdir', dest='outdir', required=True, help='output directory for plots and CSV')
    parser.add_argument('-xn', '--expect-name', dest='expect_name', help='expected distribution key label; defaults to -xa')
    parser.add_argument('-pn', '--exper-name', dest='exper_name', help='experimental distribution key label; defaults to -pa')
    parser.add_argument('-r', '--rcutoff', dest='replicate_cutoff', default=1, type=int, help='replicate observation number required; defaults to 1')
    parser.add_argument('-s', '--scutoff', dest='score_cutoff', default=30, type=int, help='MS2 score required; defaults to 30')
    parser.add_argument('-l', '--length', dest='sequon_length', default=4, type=int, help='length of sequons to match; defaults to 4')
    ...
    expected_sequon_info = get_sequoninfodict_from_files_accid(expected_csvfiles, args.expect_accid, score_cutoff=args.score_cutoff, replicate_cutoff=args.replicate_cutoff, sequon_length=args.sequon_length)
    expected_sequon_to_axis = get_distribution_axes(expected_sequon_info)

    experimental_sequon_info = get_sequoninfodict_from_files_accid(experimental_csvfiles, args.exper_accid, score_cutoff=args.score_cutoff, replicate_cutoff=args.replicate_cutoff, sequon_length=args.sequon_length)
    experimental_sequon_to_axis = get_distribution_axes(experimental_sequon_info)


    # match by sequon
    print 'Matching by sequon'
    matched_dict = get_matched_sequon_infodict(expected_sequon_info, experimental_sequon_info)

    # get plots, nKLD, and cosine similarity
    print 'Getting axes and similarity measurements'
    calculation_axes = get_comparison_calculation_dict(matched_dict, args.expect_name, args.exper_name)
    ...
    for sequon, sdict in calculation_axes.items():
        comparison_sites = '{}_{}'.format(sdict['sitepair'][0], sdict['sitepair'][1])
        fname = '{}/{}_{}.svg'.format(comparison_subdir, sequon, comparison_sites)
        save_plot(fname, sdict['comparison'])

    # save expected plots
    expected_subdir = '{}/{}_distributions'.format(args.outdir, args.expect_accid)
    ...
    experimental_subdir = '{}/{}_distributions'.format(args.outdir, args.exper_accid)
